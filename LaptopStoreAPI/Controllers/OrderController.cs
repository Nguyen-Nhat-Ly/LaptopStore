    ...
    public class OrderController : ControllerBase
    {
        ...
        public async Task<IActionResult> Add(OrderRequestModel request)
        {
            try
            {
                ...
                var result = await _serivce.Add(request, _userId);
                if(request.TransMethod == 2)
                {
                    return Ok(await _momoSerivce.QuickPay(/*await _serivce.Add(request, _userId)*/result, _userId));
                }
                else
                {
        ...
        public IActionResult GetByShipperId()
        {
            try
            {
                var claimsIdentity = this.User.Identity as ClaimsIdentity;
                string _userId = claimsIdentity.FindFirst(ClaimTypes.Name)?.Value;
                return Ok(_serivce.GetByShipperId(_userId));
            }
            catch(Exception e)
            {
                return StatusCode(500, "Fail to Get Order");
